...
`,
	...
	const settings = {
		...
		"workbench.view.extension.beandao.codai-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.beandao.codai-ActivityBar",
			when: "viewContainer.workbench.view.extension.beandao.codai-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.beandao.codai-ActivityBar');
			
			// Also try to open Cline in a tab as a fallback
			setTimeout(() => {
				require('vscode').commands.executeCommand('codai.openInNewTab');
			}, 5000);
		}, 5000);
	`
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						await vscode.commands.executeCommand('workbench.view.extension.beandao.codai-ActivityBar');

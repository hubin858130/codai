...
const downloadSources = [
    {
        url: (version, nodeAbi) => `https://github.com/TryGhost/node-sqlite3/releases/download/v${version}/sqlite3-v${version}-napi-v3-${nodeAbi.split('-')[2]}-${nodeAbi.split('-')[3]}.tar.gz`,
        name: 'TryGhost'
    },
    {
...
async function compileSqlite3(platform, arch) {
    ...
    try {
        ...
        // 安装 sqlite3
        await exec(`npm install sqlite3@${version} --build-from-source --target_platform=${platform} --target_arch=${arch}`, { env });
        
        // 复制编译后的文件到目标目录
        const sourceDir = path.join(process.cwd(), 'node_modules/sqlite3');
        const targetDir = path.join(process.cwd(), 'node_modules/sqlite3', 'lib', 'binding', `node-v${nodeAbiVersion}-${platform}-${arch}`);
...
function copyAdditionalFiles() {
    ...
    const xhrWorkerSrc = path.join(__dirname, '..', 'continue_core', 'node_modules', 'jsdom', 'lib', 'jsdom', 'living', 'xhr', 'xhr-sync-worker.js');

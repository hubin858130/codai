...
import { IndexResultType, MarkCompleteCallback, RefreshIndexResults, type CodebaseIndex } from "./types.js"
...
export class TestCodebaseIndex implements CodebaseIndex {
	...
	async *update(
		...
		for (const item of [...results.compute, ...results.addTag]) {
			await db.run("INSERT INTO test_index (path, branch, directory) VALUES (?, ?, ?)", [
				item.path,
				tag.branch,
				tag.directory,
			])
		...
		for (const item of [...results.del, ...results.removeTag]) {
			await db.run("DELETE FROM test_index WHERE path = ? AND branch = ? AND directory = ?", [
				item.path,
				tag.branch,
				tag.directory,
			])
	...
	async getIndexedFilesForTags(tags: IndexTag[]): Promise<string[]> {
		...
		const rows = await db.all(
			`SELECT path FROM test_index WHERE (branch, directory) IN (VALUES ${tags.map(() => "(?, ?)").join(", ")})`,

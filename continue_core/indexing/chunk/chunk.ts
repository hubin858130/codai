...
async function* chunkDocumentWithoutId(fileUri: string, contents: string, maxChunkSize: number): AsyncGenerator<ChunkWithoutID> {
	if (contents.trim() === "") {
		return
	}
...
export async function* chunkDocument({ filepath, contents, maxChunkSize, digest }: ChunkDocumentParam): AsyncGenerator<Chunk> {
	let index = 0

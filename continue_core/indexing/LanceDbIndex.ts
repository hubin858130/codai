...
export class LanceDbIndex implements CodebaseIndex {
	 ...
	 */
	static async create(embeddingsProvider: ILLM, readFile: (filepath: string) => Promise<string>): Promise<LanceDbIndex | null> {
	...
	async retrieve(query: string, n: number, tags: BranchAndDir[], filterDirectory: string | undefined): Promise<Chunk[]> {
		const lance = LanceDbIndex.lance!
		if (!this.embeddingsProvider) {
			return []
		}

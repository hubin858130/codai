...
import { Chunk, ContinueConfig, DocsIndexingDetails, IDE, IdeInfo, ILLM, IndexingStatus, SiteIndexingConfig } from "../.."
...
export default class DocsService {
	...
	static createSingleton(configHandler: ConfigHandler, ide: IDE, messenger?: IMessenger<ToCoreProtocol, FromCoreProtocol>) {
	...
	async indexAndAdd(siteIndexingConfig: SiteIndexingConfig, forceReindex: boolean = false): Promise<void> {
		...
		if (!forceReindex) {
			const cacheLoaded = await this.tryLoadFromCache(startUrl, provider.embeddingId, siteIndexingConfig)
		...
		try {
			...
			const chunks: Chunk[] = []
			const articleChunker = usedCrawler === "github" ? markdownPageToArticleWithChunks : htmlPageToArticleWithChunks
	 ...
	 */
	private async tryFetchFromCache(startUrl: string, embeddingId: string): Promise<boolean> {
		try {
	...
	async retrieveChunks(startUrl: string, vector: number[], nRetrieve: number, isRetry: boolean = false): Promise<Chunk[]> {
		// Get the appropriate embeddings provider
		const { provider } = await this.getEmbeddingsProvider()
		if (!provider) {
			return []
	...
	private async syncDocs(oldConfig: ContinueConfig | undefined, newConfig: ContinueConfig, forceReindex: boolean) {
		try {
			...
			const deletedDocs = currentlyIndexedDocs.filter((doc) => !newConfigStartUrls.includes(doc.startUrl))
	...
	private async createLanceDocsTable(connection: LanceType.Connection, initializationVector: number[], tableName: string) {
	...
	private async getOrCreateLanceTable({
		...
		if (!tableNames.includes(tableNameFromEmbeddingsProvider)) {
			if (initializationVector) {
				...
				console.trace(
					"No existing Lance DB docs table was found and no initialization " + "vector was passed to create one",
				)
			}
		}
	...
	private addToConfig(siteIndexingConfig: SiteIndexingConfig) {
		...
		const doesEquivalentDocExist = this.config.docs?.some((doc) => this.siteIndexingConfigsAreEqual(doc, siteIndexingConfig))
		...
		if (!doesEquivalentDocExist) {
			editConfigFile(
				...
				(config) => ({
					...config,
					docs: [
						...(config.docs?.filter((doc) => (doc as any).startUrl !== siteIndexingConfig.startUrl) ?? []),
						{

...
export class DocsCache {
	 ...
	 */
	static getFilepathForEmbeddingIdAndUrl(embeddingId: string, url: string): string {
	 ...
	 */
	static async getDocsCacheForUrl(embeddingId: string, url: string): Promise<string> {
		const filepath = DocsCache.getFilepathForEmbeddingIdAndUrl(embeddingId, url)
		...
		return new Promise<string>((resolve, reject) => {
			...
			download.on("response", (response: any) => {
				if (response.statusCode !== 200) {
					reject(new Error("There was an error retrieving the pre-indexed doc"))
				}
			})

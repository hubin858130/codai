...
class GitHubCrawler {
	...
	async *crawl(): AsyncGenerator<PageData> {
		console.debug(`[${(this.constructor as any).name}] Crawling GitHub repo: ${this.startUrl.toString()}`)
	...
	private async getGithubRepoDefaultBranch(owner: string, repo: string): Promise<string> {
		const repoInfo = await this.octokit.repos.get({ owner, repo })
		return repoInfo.data.default_branch
	}

	private async getGitHubRepoPaths(owner: string, repo: string, branch: string) {
		...
		return tree.data.tree
			.filter((file: any) => file.type === "blob" && this.markdownRegex.test(file.path ?? ""))
			.map((file: any) => file.path)
	}

	private async getGithubRepoFileContent(path: string, owner: string, repo: string) {
		try {

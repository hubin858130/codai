...
describe("DocsCache", () => {
	...
	test("normalizeEmbeddingId() produces a valid ID without constructor name", async () => {
		...
		const embeddingId = DocsCache.normalizeEmbeddingId(openAIEmbeddings.embeddingId)

		// The ID should not contain the constructor name (OpenAI)
		expect(embeddingId).toEqual(`${openAIEmbeddings.model}::${openAIEmbeddings.maxEmbeddingChunkSize}`)
	})
})

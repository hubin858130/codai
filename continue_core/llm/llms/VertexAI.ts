...
class VertexAI extends BaseLLM {
	static override providerName = "vertexai"
	...
	override async fetch(url: RequestInfo | URL, init?: RequestInit) {
	...
	protected async *StreamChatAnthropic(messages: ChatMessage[], options: CompletionOptions): AsyncGenerator<ChatMessage> {
	...
	private async *streamChatGemini(messages: ChatMessage[], options: CompletionOptions): AsyncGenerator<ChatMessage> {
		...
		for await (const message of this.geminiInstance.processGeminiResponse(streamResponse(response))) {
			yield message
		}
	}
	...
	protected async *StreamChatMistral(messages: ChatMessage[], options: CompletionOptions): AsyncGenerator<ChatMessage> {

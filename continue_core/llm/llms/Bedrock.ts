...
class Bedrock extends BaseLLM {
	static override providerName = "bedrock"
	...
	public override requestOptions: {
		region?: string
		credentials?: any
			...
			for await (const chunk of response.stream) {
				...
				if (chunk.contentBlockDelta?.delta) {
					...
					if ((chunk.contentBlockDelta.delta as any).reasoningContent?.text) {
						yield {
							role: "thinking",
							content: (chunk.contentBlockDelta.delta as any).reasoningContent.text,
						}
						continue
					}
	...
	// EMBED //
	override async embed(chunks: string[]): Promise<number[][]> {
		const credentials = await this._getCredentials()
		const client = new BedrockRuntimeClient({
			region: this.region,
			credentials: {
	...
	override async rerank(query: string, chunks: Chunk[]): Promise<number[]> {
		...
		try {
			...
			return responseBody.results.sort((a: any, b: any) => a.index - b.index).map((result: any) => result.relevance_score)
		} catch (error: unknown) {
			if (error instanceof Error) {
				if ("code" in error) {
					// AWS SDK specific errors

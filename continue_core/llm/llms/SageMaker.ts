...
class SageMaker extends BaseLLM {
	...
	static override providerName = "sagemaker"
	...
	override async embed(chunks: string[]) {
		const credentials = await this._getCredentials()
		const client = new SageMakerRuntimeClient({
			region: this.region,
			credentials: {
...
interface SageMakerModelToolkit {
	generateCommand(messages: ChatMessage[], prompt: string, options: CompletionOptions): InvokeEndpointWithResponseStreamCommand
}

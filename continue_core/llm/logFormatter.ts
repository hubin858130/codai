...
export class LLMLogFormatter {
	...
	private getInteractionData(item: LLMInteractionItem) {
		...
		while (true) {
			...
			if (!usedPrefixes.includes(candidate) && (candidate === " " || candidate !== this.lastFreedPrefix)) {
				prefix = candidate
				break
			}
			i++
	...
	private formatTimestamp(interaction: InteractionData, item: LLMInteractionItem) {
	...
	private logFragment(
		...
		if (this.openLine && (!joinBefore || item.interactionId !== this.lastItem?.interactionId)) {
			this.openLine = false
		...
		let continueAt = null
		if (wrap && fragment.length - startAt > this.wrapWidth - this.openLineChars) {
	...
	private logLines(item: LLMInteractionItem, content: string, marker: string = "") {
	...
	private logMessageContent(item: LLMInteractionItem, message: AssistantChatMessage | UserChatMessage | ThinkingChatMessage) {
	...
	private logMessage(item: LLMInteractionItem, message: ChatMessage, forceRole: boolean = false) {
		let showRole = true
		if (!forceRole && (message.role === "assistant" || message.role === "thinking")) {
			const interaction = this.getInteractionData(item)
			const lastMessage = interaction.lastItem?.kind === "message" ? interaction.lastItem.message : null
	...
	private logTokens(item: LLMInteractionSuccess | LLMInteractionError | LLMInteractionCancel) {
	...
	private logOptions(item: LLMInteractionStartChat | LLMInteractionStartComplete | LLMInteractionStartFim) {
	...
	private logItem(item: LLMInteractionItem) {
		...
		if (item.kind === "cancel" || item.kind === "error" || item.kind === "success") {

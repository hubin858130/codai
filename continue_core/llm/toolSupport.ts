export const PROVIDER_TOOL_SUPPORT: Record<string, (model: string) => boolean | undefined> = {
	...
	anthropic: (model) => {
		if (["claude-3-5", "claude-3.5", "claude-3-7", "claude-3.7"].some((part) => model.toLowerCase().startsWith(part))) {
			return true
		}
		return false
	},
	...
	ollama: (model) => {
		...
		if (
			["vision", "math", "guard", "mistrallite", "mistral-openorca"].some((part) => modelName.toLowerCase().includes(part))
		) {
			return false
		}
		if (

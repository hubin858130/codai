...
export abstract class BaseLLM implements ILLM {
	...
	constructor(_options: LLMOptions) {
		...
		const modelSearchString = this.providerName === "continue-proxy" ? this.model?.split("/").pop() || this.model : this.model
	...
	async *streamFim(
		...
		try {
			...
			status = this._logEnd(completionOptions.model, fimLog, completion, undefined, interaction)
		} catch (e) {
			status = this._logEnd(completionOptions.model, fimLog, completion, undefined, interaction, e)
			throw e
		} finally {
			if (status === "in_progress") {
				this._logEnd(completionOptions.model, fimLog, completion, undefined, interaction, "cancel")
			}
		}
	...
	async *streamComplete(_prompt: string, signal: AbortSignal, options: LLMFullCompletionOptions = {}) {
		...
		try {
			...
			status = this._logEnd(completionOptions.model, prompt, completion, undefined, interaction)
		} catch (e) {
			status = this._logEnd(completionOptions.model, prompt, completion, undefined, interaction, e)
			throw e
		} finally {
			if (status === "in_progress") {
				this._logEnd(completionOptions.model, prompt, completion, undefined, interaction, "cancel")
			}
		}
	...
	async *streamChat(
		...
		try {
			...
			status = this._logEnd(completionOptions.model, prompt, completion, thinking, interaction)
		} catch (e) {
			status = this._logEnd(completionOptions.model, prompt, completion, thinking, interaction, e)
			throw e
		} finally {
			if (status === "in_progress") {
				this._logEnd(completionOptions.model, prompt, completion, undefined, interaction, "cancel")
    ...
    */
	...
	protected async *_streamComplete(prompt: string, signal: AbortSignal, options: CompletionOptions): AsyncGenerator<string> {
		throw new Error("Not implemented")
	}
	...
	public renderPromptTemplate(
		...
		if (typeof rendered !== "string" && rendered[rendered.length - 1]?.role === "assistant" && !canPutWordsInModelsMouth) {

...
export async function* llmStreamChat(
	...
	const { title, legacySlashCommandData, completionOptions, messages } = msg.data
	...
	if (legacySlashCommandData) {
		...
		const gen = model.streamChat(messages, new AbortController().signal, completionOptions)
		let next = await gen.next()
		while (!next.done) {
			if (abortedMessageIds.has(msg.messageId)) {
				abortedMessageIds.delete(msg.messageId)

...
async function collectDiffs(
	...
	for (const diffLine of (await deterministicApplyLazyEdit(oldFile, newFile, filename)) ?? []) {
...
function displayDiff(diff: DiffLine[]) {
	return diff.map(({ type, line }) => (type === "same" ? line : `${UNIFIED_DIFF_SYMBOLS[type]} ${line}`)).join("\n")
}
...
async function expectDiff(file: string) {
	...
	const [oldFile, newFile, expectedDiff] = testFileContents.split("\n---\n").map((s) => s.replace(/^\n+/, "").trimEnd())

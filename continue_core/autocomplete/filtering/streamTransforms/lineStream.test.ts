...
describe("lineStream", () => {
	...
	describe("skipPrefixes", () => {
		it("should skip specified prefixes", async () => {
			const linesGenerator = await getLineGenerator([`${lineStream.PREFIXES_TO_SKIP[0]}const x = 5;`, "let y = 10;"])
	...
	describe("skipLines", () => {
		it("should skip specified lines", async () => {
			const linesGenerator = await getLineGenerator([`${lineStream.LINES_TO_SKIP[0]}const x = 5;`, "let y = 10;"])
	...
	describe("filterEnglishLinesAtStart", () => {
		it("should skip initial empty line", async () => {
			const linesGenerator = await getLineGenerator(["", "const x = 5;", "let y = 10;"])
		...
		it("should filter out English first line", async () => {
			const linesGenerator = await getLineGenerator([lineStream.ENGLISH_START_PHRASES[0], "let y = 10;"])
		...
		it("should filter out empty newline after first line english word", async () => {
			const linesGenerator = await getLineGenerator([lineStream.ENGLISH_START_PHRASES[0], "", "const x = 5;"])
	...
	describe("filterEnglishLinesAtEnd", () => {
		it("should stop at English explanation after code block", async () => {
			const linesGenerator = await getLineGenerator(["const x = 5;", "```", lineStream.ENGLISH_POST_PHRASES[0]])
	...
	describe("stopAtRepeatingLines", () => {
		it("should handle non-repeating lines correctly", async () => {
			const linesGenerator = await getLineGenerator(["const x = 5;", "let y = 10;", "const z = 15;"])

			const result = lineStream.stopAtRepeatingLines(linesGenerator, mockFullStop)
			const filteredLines = await getFilteredLines(result)

			expect(filteredLines).toEqual(["const x = 5;", "let y = 10;", "const z = 15;"])
		})

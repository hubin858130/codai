...
function getLastUserHistory(history: ChatMessage[]): string {
	const lastUserHistory = history.reverse().find((message) => message.role === "user")
	...
	if (Array.isArray(lastUserHistory.content)) {
		return lastUserHistory.content.reduce((acc: string, current: { type: string; text?: string }) => {
			return current.type === "text" && current.text ? acc + current.text : acc
		}, "")
	}

	return typeof lastUserHistory.content === "string" ? lastUserHistory.content : ""
}
...
const ReviewMessageCommand: SlashCommand = {
	...
	run: async function* ({ llm, history }) {
		...
		for await (const chunk of llm.streamChat([{ role: "user", content: content }], new AbortController().signal)) {

 ...
 *  escaping issues.
 */
const replaceFilepaths = (value: string, ctxProviderNames: string[]): [string, { [key: string]: string }] => {
	...
	for (const i in ast.body) {
		...
		if (node.type === "MustacheStatement") {
			...
			if (isFilepath) {
				...
				value = value.replace(new RegExp(`{{\\s*${originalNodeVal}\\s*}}`), `{{${letter}}}`)
...
export function registerHelpers(helpers: Array<[string, Handlebars.HelperDelegate]>): {
...
export async function prepareTemplateAndData(
	...
	for (const [replacedName, originalCtxProviderName] of Object.entries(vars)) {
		data[replacedName] = inputData[originalCtxProviderName] ?? originalCtxProviderName

...
export function slashCommandFromPromptFileV1(path: string, content: string): SlashCommand | null {
	const { name, description, systemMessage, prompt } = parsePromptFileV1V2(path, content)
	...
	return {
		...
		run: async function* (context) {
			...
			const messages = replaceSlashCommandWithPromptInChatHistory(context.history, name, renderedPrompt, systemMessage)

			for await (const chunk of context.llm.streamChat(messages, new AbortController().signal, context.completionOptions)) {

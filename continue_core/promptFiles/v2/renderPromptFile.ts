...
export async function renderPromptFileV2(rawContent: string, extras: ContextProviderExtras): Promise<[ContextItem[], string]> {
	...
	const renderedPrompt = contextItems.map((item) => item.content).join("\n\n") + "\n\n" + renderedBody

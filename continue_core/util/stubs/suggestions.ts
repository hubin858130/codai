...
import { openEditorAndRevealRange, translate } from "../vscode"
...
export function rerenderDecorations(editorUri: string) {
	...
	const editor = vscode.window.visibleTextEditors.find((editor) => URI.equal(editor.document.uri.toString(), editorUri))
...
function selectSuggestion(accept: SuggestionSelectionOption, key: SuggestionRanges | null = null) {
	...
	for (const below of suggestions) {
		...
		if (below.oldRange.union(below.newRange).start.line > suggestion.oldRange.union(suggestion.newRange).start.line) {

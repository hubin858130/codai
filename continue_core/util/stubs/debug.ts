...
export function registerDebugTracker(webviewProtocol: VsCodeWebviewProtocol, ide: VsCodeIde) {
						...
						switch (message.event) {
							...
							case "stopped":
								if (typeof message.body.threadId !== "undefined") {
									threadStopped.set(Number(message.body.threadId), message.event === "stopped")
								}

								if (message.body.allThreadsStopped) {
									threadStopped.forEach((_, key) => threadStopped.set(key, true))
								}

								if (message.body.allThreadsContinued) {
									threadStopped.forEach((_, key) => threadStopped.set(key, false))
								}

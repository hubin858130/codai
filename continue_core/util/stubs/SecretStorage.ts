...
export class SecretStorage {
  ...
  async encrypt(data: string, filePath: string): Promise<void> {
    ...
    const result = Uint8Array.from(Buffer.concat([Uint8Array.from(salt), Uint8Array.from(iv), Uint8Array.from(tag), Uint8Array.from(encrypted)]));
  ...
  async decrypt(filePath: string): Promise<string> {
    ...
    const encrypted = Uint8Array.from(data.subarray(
      this.saltLength + this.ivLength + this.tagLength,
    ));

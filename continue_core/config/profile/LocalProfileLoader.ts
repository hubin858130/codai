...
export default class LocalProfileLoader implements IProfileLoader {
	...
	constructor(
		...
		private overrideAssistantFile?: { path: string; content: string } | undefined,
	) {
		const description: ProfileDescription = {
			...
			iconUrl: "",
			title: overrideAssistantFile?.path ? getUriPathBasename(overrideAssistantFile.path) : "Local Assistant",
			errors: undefined,
			uri: overrideAssistantFile?.path ?? localPathToUri(getPrimaryConfigFilePath()),
			rawYaml: undefined,
		}
		this.description = description
		if (overrideAssistantFile?.content) {
			try {
				const parsedAssistant = parseConfigYaml(overrideAssistantFile?.content ?? "")

...
async function intermediateToFinalConfig(
	...
	if (config.tabAutocompleteModel) {
		tabAutocompleteModels = (
			await Promise.all(
				(Array.isArray(config.tabAutocompleteModel) ? config.tabAutocompleteModel : [config.tabAutocompleteModel]).map(
					async (desc) => {
						if (isModelDescription(desc)) {
							const llm = await llmFromDescription(
								desc,
	...
	mcpManager.setConnections(
		(config.experimental?.modelContextProtocolServers ?? []).map((server, index) => ({
...
async function finalToBrowserConfig(final: ContinueConfig, ide: IDE): Promise<BrowserSerializedContinueConfig> {
	return {
		...
		slashCommands: final.slashCommands?.map(({ run, ...slashCommandDescription }) => slashCommandDescription),
		...
		selectedModelByRole: Object.fromEntries(
			Object.entries(final.selectedModelByRole).map(([k, v]) => [k, v ? llmToSerializedModelDescription(v) : null]),
...
export { finalToBrowserConfig, intermediateToFinalConfig, loadContinueConfigFromJson, type BrowserSerializedContinueConfig }

...
import { ContinueConfig, IContextProvider, IDE, IdeInfo, IdeSettings, ILLMLogger } from "../.."
...
async function configYamlToContinueConfig(
	...
	if (ideInfo.ideType === "vscode" && !continueConfig.modelsByRole.embed.find((m) => m.providerName === "transformers.js")) {
	...
	const DEFAULT_CONTEXT_PROVIDERS = [new FileContextProvider({}), new CodebaseContextProvider(codebaseContextParams)]
	...
	if (continueConfig.docs?.length && !continueConfig.contextProviders?.some((cp) => cp.description.title === "docs")) {
...
export async function loadContinueConfigFromYaml(
	...
	const rawYaml =
		overrideConfigYaml === undefined ? fs.readFileSync(configYamlPath ?? getConfigYamlPath(ideInfo.ideType), "utf-8") : ""

	const configYamlResult = await loadConfigYaml(rawYaml, overrideConfigYaml, controlPlaneClient, orgScopeId, ideSettings)

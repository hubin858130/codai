...
export async function getAssistantFilesFromDir(ide: IDE, dir: string): Promise<{ path: string; content: string }[]> {
	try {
		...
		const assistantFilePaths = uris.filter((p) => p.endsWith(".yaml") || p.endsWith(".yml"))
...
export async function getAllAssistantFiles(ide: IDE): Promise<{ path: string; content: string }[]> {
	...
	assistantFiles = (await Promise.all(fullDirs.map((dir) => getAssistantFilesFromDir(ide, dir)))).flat()

...
export class ConfigHandler {
	...
	constructor(
		...
		this.globalLocalProfileManager = new ProfileLifecycleManager(
			new LocalProfileLoader(ide, ideSettingsPromise, this.controlPlaneClient, this.llmLogger),
			this.ide,
		)
	...
	private async getOrgs(): Promise<OrgWithProfiles[]> {
		...
		if (userId) {
			...
			if (await useHub(this.ideSettingsPromise)) {
				...
				const hubOrgs = await Promise.all(orgDescs.map((org) => this.getNonPersonalHubOrg(org)))
	...
	private async getNonPersonalHubOrg(org: OrganizationDescription): Promise<OrgWithProfiles> {
	...
	async getAllLocalProfiles() {
		...
		const profiles = assistantFiles.map((assistant) => {
			return new LocalProfileLoader(this.ide, this.ideSettingsPromise, this.controlPlaneClient, this.llmLogger, assistant)
		})

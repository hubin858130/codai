...
function createAnalyticsProvider(config: AnalyticsConfig): IAnalyticsProvider | undefined {
	// @ts-ignore
...
export class TeamAnalytics {
	...
	static async setup(
		...
		if (config.provider === "continue-proxy") {
			;(TeamAnalytics.provider as ContinueProxyAnalyticsProvider).controlPlaneClient = controlPlaneClient
		}
	}

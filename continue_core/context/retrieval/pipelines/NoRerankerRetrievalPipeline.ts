...
export default class NoRerankerRetrievalPipeline extends BaseRetrievalPipeline {
	override async run(args: RetrievalPipelineRunArguments): Promise<Chunk[]> {
		...
		try {
			embeddingsChunks = !!config.selectedModelByRole.embed ? await this.retrieveEmbeddings(input, embeddingsNFinal) : []
		} catch (error) {

...
import { ContextProviderExtras, ContextProviderWithParams, IContextProvider } from "../.."
...
async function getContextProviderExtras(fullInput: string): Promise<ContextProviderExtras> {
	const ide = new FileSystemIde(TEST_DIR)
	const ideSettingsPromise = ide.getIdeSettings()
	const llmLogger = new LLMLogger()
	const configHandler = new ConfigHandler(ide, ideSettingsPromise, llmLogger, Promise.resolve(undefined))

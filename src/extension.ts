...
export async function activate(context: vscode.ExtensionContext) {
	outputChannel = vscode.window.createOutputChannel("Codai")
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`Codai version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			const lastShownPopupNotificationVersion = context.globalState.get<string>("codaiLastPopupNotificationVersion")
			...
			if (currentVersion !== lastShownPopupNotificationVersion && previousVersion) {
				...
				const message = `Codai has been updated to v${currentVersion}`
				await vscode.commands.executeCommand("codai-top.SidebarProvider.focus")
	...
	*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codai.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			await vscode.commands.executeCommand("codai.focusChatInput") // Ensure Codai is visible and input focused
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codai.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Codai (Always available)
					const addAction = new vscode.CodeAction("Add to Codai", vscode.CodeActionKind.QuickFix)
					addAction.command = {
					...
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Codai", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: "codai.fixWithCodai",
							title: "Fix with Codai",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codai.explainCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("codai.focusChatInput") // Ensure Cline is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
				return
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codai.improveCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("codai.focusChatInput") // Ensure Cline is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codai.focusChatInput", async () => {
			...
			if (activeWebviewProvider?.getWebview() && activeWebviewProvider.getWebview().hasOwnProperty("reveal")) {
				...
				if (!activeWebviewProvider) {
					...
					await vscode.commands.executeCommand("codai.openInNewTab")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codai.openWalkthrough", async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", "beandao.codai-top#ClineWalkthrough")

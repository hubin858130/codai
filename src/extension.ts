...
export function activate(context: vscode.ExtensionContext) {
	try {
		...
		console.log("WebviewProvider 创建成功")
		...
		*/
		...
		context.subscriptions.push(
			vscode.commands.registerCommand("codai.focusChatInput", () => {
				let visibleWebview = WebviewProvider.getVisibleInstance()
				if (!visibleWebview) {
					vscode.commands.executeCommand("codai-top.SidebarProvider.focus")
		...
		console.log("Codai 扩展激活成功")
		return createClineAPI(outputChannel, sidebarWebview.controller)
	} catch (error) {
		console.error("Codai 扩展激活失败:", error)
		vscode.window.showErrorMessage(`Codai 扩展激活失败: ${error.message}`)
		// 返回一个空的 API 对象，避免调用者出错
		return {
			// 添加必要的空方法
			dispose: () => {},
			// 其他必要的空方法...

...
export async function activate(context: vscode.ExtensionContext) {
	...
	const openClineInNewTab = async () => {
		...
		const panel = vscode.window.createWebviewPanel(VscodeWebviewProvider.TAB_PANEL_ID, "Codai", targetCol, {
			enableScripts: true,
	...
	*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codai.generateGitCommitMessage", async (scm) => {
			await GitCommitGenerator?.generate?.(context, scm)
		}),
		vscode.commands.registerCommand("codai.abortGitCommitMessage", () => {
...
function setupHostProvider(context: ExtensionContext) {
	...
	const getCallbackUri = async () => `${vscode.env.uriScheme || "vscode"}://beandao.codai-top`
